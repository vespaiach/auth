...
// MysqlUserRepo will implement models.UserRepo
type MysqlUserRepo struct {
	DbClient *gorm.DB
}

// NewMysqlUserRepo create new instance of MysqlUserRepo
func NewMysqlUserRepo(db *gorm.DB) models.UserRepo {
...
func (repo *MysqlUserRepo) GetByID(id uint) (user *models.User, err error) {
	user = new(models.User)
	repo.DbClient.Where("id=?", id).First(&user)

	if user == nil {
		err = datatypes.ErrDataNotFound
	}
...
func (repo *MysqlUserRepo) GetByUsername(username string) (user *models.User, err error) {
	user = new(models.User)
...
func (repo *MysqlUserRepo) GetByEmail(email string) (user *models.User, err error) {
	user = new(models.User)
...
// Create a new user
func (repo *MysqlUserRepo) Create(fullName string, username string, hashed string, email string) (*models.User, error) {
	user := models.User{
		FullName: fullName,
		Username: username,
		Hashed:   hashed,
	...
	if repo.DbClient.NewRecord(user) {
		return nil, datatypes.ErrCreadDataFailed
	}
...
func (repo *MysqlUserRepo) Query(page int, perPage int, filters map[string]interface{}, sorts map[string]datatypes.SortDirection) ([]*models.User, error) {
	db := repo.DbClient.Model(&models.User{})
		...
		if ok {
			if k == "active" {
				...
				if good {
					...
					return nil, datatypes.ErrDataTypeMismatch
				}
			}
		} else {
			return nil, datatypes.ErrNotAllowField
		}
	}
